...
```text
App.tsx                         الاكتشاف أولًا، والقروبات عند اختيارها، أو رابط الدعوة
src/
...
| `groups` و`members` | الخطة المباشرة ورفقتها بمفاتيح وصول، وتشمل بيانات النسخة السابقة؛ لا تتطلب حسابًا أو circle |
| `schema_migrations` | أسماء الترحيلات وبصماتها وتواريخ تطبيقها |

المجموع ١٥ جدولًا، منها ١٢ أضيفت في الترحيل الثاني. التسمية `circles` تميز القروبات الدائمة عن جدول `groups` للخطة المباشرة، بينما اسمها في API هو `groups`. لم تتغير العلاقات أو الأعمدة في تصحيح البداية؛ المعنى والاستخدام موضحان هنا دون ترحيل أو إعادة تسمية للبيانات.
