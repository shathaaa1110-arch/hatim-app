# معمارية حاتم السابقة — لقطة 59d45d3

هذه لقطة تاريخية قبل إضافة القروبات الثابتة. [المعمارية الحالية](architecture-python-postgres.ar.md). الفرع المرجعي: `codex/python-postgres`. نسخة .NET ودليلها محفوظان في [فرع مستقل](https://github.com/shathaaa1110-arch/hatim-app/tree/codex/dotnet-backend).
...
نقاش الإضافة قبل تنفيذها: [قروبات ثابتة وطلعات وتصويت وأدوار وقرعة ومزاح](proposals/persistent-groups.ar.md). لها معمارية مستهدفة وERD وتسلسل حسم مستقل، كانت **غير منفذة في هذه اللقطة**؛ الجداول والرسوم أدناه تصف المصدر القديم. عند تنفيذ كل مرحلة تُحدّث هذه الصفحة والعقد والشرح مع الكود.
