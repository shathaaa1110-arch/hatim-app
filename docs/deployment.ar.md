...
## الدعوات وGoogle —25سبتمبر2026

النشر يخدم `/s/{code}` من نفس حاوية الويب وAPI، وتطبق006 و007 عند بدء الخادم.006 يضيف الأطباق للتجارب الحالية فقط عند غيابها؛007 يضيف جدول الدعوات وتسلسل مراجعاتها. PDF ينشأ على جهاز المستخدم ولا يحتاج خادم طباعة أو مساحة ملفات دائمة. اسم النطاق في PDF ثابت وقت إنشائه؛ مع Quick Tunnel يتوقف رابط النسخة القديمة عند تغير النطاق، بينما رموز الدعوات المحفوظة تبقى صالحة تحت النطاق الجديد.

GOOGLE_PLACES_API_KEY اختياري في deploy/.env.local؛ Compose يمرره للخادم فقط. لا تضعيه في EXPO_PUBLIC. فعّلي Places API في مشروع Google واضبطي القيود والحصة من حسابك قبل الاستعمال، واربطِي فقط أماكن حقيقية مؤكدة في الكتالوج مع is_demo=false وgoogle_place_id. الخدمة الحالية لا توفر محرر كتالوج داخل التطبيق. حد20طلبًا في الدقيقة محلي لكل عملية؛ حصة Google Cloud هي الضبط المشترك عند تعدد العمال. لم يُفعّل مفتاح أو إنفاق فعلي في هذه الإضافة. [سلوك الحالات الغائبة وعرض المصدر](api-plan-sharing.ar.md).
