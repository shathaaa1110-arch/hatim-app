...
from hatim.core.models import TitleChange
from hatim.domain.models import Preferences
from hatim.features.accounts import User
from hatim.integrations.legacy_plans import claim_legacy as convert_legacy
...
def claim_legacy(body: LegacyClaim, user: User):
    with connect() as db:
        circle_id = convert_legacy(db, body.group_id, body.owner_token.get_secret_value(), user.id)
        return circle_view(db, *circle_access(db, circle_id, user.id))
