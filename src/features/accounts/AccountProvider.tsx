...
function useAccountState() {
  ...
  const restore = useCallback(async () => {
    ...
    try {
      ...
      setSession(
        token ? { token, account: await accountsApi.me(token) } : null,
      );
    } catch (e) {
  ...
  const signOut = async () => {
    if (session) await accountsApi.logout(session.token);
    await storage.remove(sessionKey);
    setSession(null);
  };
  const updateName = async (name: string) => {
    if (!session) return;
    const account = await accountsApi.updateMe(session.token, name);
