...
import { PlanScreen } from "../planning";
...
export function OutingScreen({
  ...
  const attend = (value: "going" | "pending" | "declined") => {
    ...
    setLocalError(null);
    void update(() => groupsApi.attendance(token, id, value, parsed)).catch(
      () => {},
    );
  };
        ...
        <>
          ...
          {o.cards.map((card) => (
            <Bench
              ...
              busy={r.busy}
              dismiss={() => {
                void update(() => groupsApi.dismissFun(token, id)).catch(
                  () => {},
                );
              }}
            />
                ...
                <Button
                  ...
                  onPress={() =>
                    setConfirm({
                      ...
                      label: "إنهاء وحفظ",
                      run: () => update(() => groupsApi.closeOuting(token, id)),
                    })
                  }
                />
              )}
