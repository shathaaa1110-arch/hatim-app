...
import { colors as c, ar, photos } from "../../shared/theme";
import {
  Button,
  Chip,
  Empty,
...
export function InviteScreen({ code }: { code: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [next, experiences, stored] = await Promise.all([
        plansApi.invite(code),
        experiencesApi.catalog(),
      ...
      if (stored) {
        try {
          setMember(await plansApi.me(code, stored));
          setToken(stored);
        } catch (e) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      fetching = true;
      try {
        const next = await plansApi.invite(code);
        if (alive) {
          setInvite(next);
          setError(null);
        }
        if (token) {
          try {
            const me = await plansApi.me(code, token);
  ...
  const save = async (preferences: Preferences) => {
    ...
    setBusy(true);
    try {
      if (token) setMember(await plansApi.updateMe(code, token, preferences));
      else {
        const result = await plansApi.join(code, preferences);
      ...
      setEdit(false);
      try {
        setInvite(await plansApi.invite(code));
        setError(null);
      } catch (e) {
        setError(
