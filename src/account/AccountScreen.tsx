...
import { api, ApiError, type PlanSummary } from "../api/client";
import { Button, Notice, Row, Sheet, T } from "../components/ui";
import { readSession, storage } from "../storage";
import { ar } from "../theme";
import { AuthScreen } from "../social/AuthScreen";
import { ErrorNotice, Field, Loading, Page, Panel, s } from "../social/ui";
import { useRemote } from "../social/useRemote";
import { type AccountSession } from "../social/client";
import { planKey, useAccount } from "./AccountProvider";
...
export function AccountScreen(props: Navigation) {
...
function SignedIn({
  ...
  const read = useCallback(async () => {
    const plans = await api.plans(session.token).catch(async (e: unknown) => {
    ...
    if (saved) {
      try {
        guest = { session: saved, group: await api.group(saved) };
      } catch (e) {
          ...
          <Button
            ...
            onPress={() => {
              void run(async () => {
                ...
                if (!guest) return;
                const saved = await api.saveToAccount(
                  guest.session,
                  session.token,
                );
            ...
            void run(async () => {
              ...
              if (edit.kind === "name") await account.updateName(value.trim());
                ...
                await r.mutate(async () => {
                  if (edit.kind === "rename")
                    await api.rename(owner, value.trim());
                  else {
                    await api.deletePlan(owner);
                    if (
