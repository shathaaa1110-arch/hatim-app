...
**Designed invitations:** direct plans and managed group outings offer **دعوة ومشاركة الخطة**: choose one of three designs, preview the public details, then create a revocable `/s/<code>` link. It opens without an account and follows the current plan, keeping personal preferences and member names private. An optional Arabic PDF snapshot includes suggested dishes and clickable Maps/current-plan links. Read [the sharing API](docs/api-plan-sharing.ar.md) and [chapter20](docs/learning-python-postgres/20-designed-invitations.ar.md). The separate `/join` flow still collects companions' preferences.
